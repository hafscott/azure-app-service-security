...
namespace Benday.EasyAuthDemo.UnitTests.DomainModels
{
    public class DomainModelFieldTester<T> where T : DomainModelBase
    {
        ...
        public DomainModelFieldTester(T instance)
        {
            if (instance == null)
            {
                throw new ArgumentNullException(nameof(instance), "Argument cannot be null.");
            }
            
            _SystemUnderTest = instance;
        }
        ...
        private void AssertAllPropertiesReturnToAcceptedChangeOnUndoChanges(
            List<PropertyInfo> propertiesToTest)
        {
            foreach (var prop in propertiesToTest)
            {
                AssertPropertyReturnsToAcceptedChangeOnUndoChanges(prop);
        ...
        private void ModifyValue(PropertyInfo prop, MethodInfo setter, MethodInfo getter)
        {
            ...
            {
                modifiedValue = DateTime.Now.AddYears(10);
            }
        ...
        {
            return getter.Invoke(_SystemUnderTest, new object[] { });
        }
    }
}
